...
QUEST=/mnt/c/Users/Danie/Downloads/Civs-1.11.6/rpg-server-plugin/src/main/resources/quests/sprint3_daily.yml
TS=$(date +%Y%m%d-%H%M)
...
scp -o BatchMode=yes "$QUEST" "${HOST}:${PLUGINS}/RPGServer/quests/sprint3_daily.yml"
...
echo "=== RPGServer quest lines ==="
ssh -o BatchMode=yes "$HOST" "grep -iE 'RPGServer|quest' '$SERVER/logs/latest.log' | tail -50"
